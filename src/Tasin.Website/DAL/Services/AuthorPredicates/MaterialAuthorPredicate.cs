...
{
    public static class MaterialAuthorPredicate
    {
        public static Expression<Func<Material, bool>> GetMaterialAuthorPredicate(Expression<Func<Material, bool>> predicate, List<ERoleType> roleList, int currentUserId = -1)
        {
            var predicateInner = PredicateBuilder.New<Material>(predicate);
            ...
            {
                // User chỉ có thể xem những vật liệu do mình tạo
                if (currentUserId > 0)
                {
