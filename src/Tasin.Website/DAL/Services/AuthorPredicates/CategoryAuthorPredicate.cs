...
{
    public static class CategoryAuthorPredicate
    {
        public static Expression<Func<Category, bool>> GetCategoryAuthorPredicate(Expression<Func<Category, bool>> predicate, List<ERoleType> roleList, int currentUserId = -1)
        {
            var predicateInner = PredicateBuilder.New<Category>(predicate);
            ...
            {
                // User chỉ có thể xem những danh mục do mình tạo
                if (currentUserId > 0)
                {
